...
from app.workflows.task_breakdown_workflow import TaskBreakdownWorkflow
...
class TaskBreakdownService:
    ...
    def start_generation(self, project_id: str, background_tasks: BackgroundTasks):
        ...
        run = self.run_repository.create(project_id=project_id, run_type=RunType.TASK_BREAKDOWN_GENERATION, status=RunStatus.PENDING)
        background_tasks.add_task(run_task_breakdown_generation_task, run.id, project_id)
        return run


def run_task_breakdown_generation_task(run_id: str, project_id: str) -> None:
    db = SessionLocal()
    try:
