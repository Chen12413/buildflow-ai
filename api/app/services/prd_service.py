...
from app.workflows.prd_workflow import PRDWorkflow
...
class PRDService:
    ...
    def start_generation(self, project_id: str, background_tasks: BackgroundTasks):
        ...
        run = self.run_repository.create(project_id=project_id, run_type=RunType.PRD_GENERATION, status=RunStatus.PENDING)
        background_tasks.add_task(run_prd_generation_task, run.id, project_id)
        return run


def run_prd_generation_task(run_id: str, project_id: str) -> None:
    db = SessionLocal()
    try:
