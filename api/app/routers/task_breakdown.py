...
router = APIRouter(prefix="/projects/{project_id}/task-breakdown", tags=["task-breakdown"])


@router.post("/generate", status_code=status.HTTP_202_ACCEPTED)
def generate_task_breakdown(project_id: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
